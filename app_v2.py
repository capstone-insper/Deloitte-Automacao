...
# Clique vindo do histórico do assistente na sidebar — agenda troca de aba.
if st.session_state.get("_jump_to_assistant", False):
    st.session_state._jump_to_assistant = False
    st.session_state["_pending_tab_switch"] = 9

# ─────────────────────────────────────────────────────────────────────────────
# ABAS NATIVAS (st.tabs) — responsivas com scroll horizontal automático
# ─────────────────────────────────────────────────────────────────────────────
...
# Injeção JS para navegação programática (ex.: botão "Novo insight" → Assistente)
if _pending_switch is not None:
    import streamlit.components.v1 as _components
    _components.html(f"""
    <script>
    (function() {{
        var idx = {_pending_switch};
        var attempts = 0;
        function tryClick() {{
